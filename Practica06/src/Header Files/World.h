...
struct SLayer {
  const ltex_t* m_pBackground;

  float m_fScrollRatio;
  CVec2 m_vScrollSpeed;
};
...
public:
  ...
  const ltex_t* GetBackground(size_t _iLayer) const;

  float GetScrollRatio(size_t _iLayer) const;
  void SetScrollRatio(size_t _iLayer, float _fRatio);
  const CVec2& GetScrollSpeed(size_t _iLayer) const;
  void SetScrollSpeed(size_t _iLayer, const CVec2& _rSpeed);
